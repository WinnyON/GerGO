...
class Repository():
	...
	def select_rows(self, db_name, table_name, columns, join_tables, conditions, aliases, aggregates, distinct_columns, group_by, limit_count, order_by):
		select_all = False
		try:
			...
			for col in group_by:
				if '.' not in col:
					command = "22^" + table_name + "^" + col
				else:
					table1, col1 = col.split('.')
					table_name1 = table1
					if table1 in aliases.keys():
						table_name1 = aliases[table1]
					command = "22^" + table_name1 + "^" + col1
				code = self.client.send_message(command)
				if code[0] == '1':
					self.client.set_timeout()
					return 1, code.split('^')[1]
			...
			for col, order in order_by:
				if '.' not in col:
					command = "24^" + table_name + "^" + col + "^" + order
				else:
					table1, col1 = col.split('.')
					table_name1 = table1
					if table1 in aliases.keys():
						table_name1 = aliases[table1]
					command = "24^" + table_name1 + "^" + col1 + "^" + order
				code = self.client.send_message(command)
				if code[0] == '1':
					self.client.set_timeout()
					return 1, code.split('^')[1]
			...
			if limit_count != '':
				command = "27^" + limit_count
				code = self.client.send_message(command)
				if code[0] == '1':
					self.client.set_timeout()
					return 1, code.split('^')[1]

			# end of select query data
			command = "0^OK"
			code = self.client.send_message(command)
			if code[0] == '1':
				self.client.set_timeout()
